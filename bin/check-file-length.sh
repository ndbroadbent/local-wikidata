...
check_file() {
  ...
  if [[ "$file" == *_test.py ]] || [[ "$file" == *test_*.py ]] || [[ "$file" == *.test.ts ]] || [[ "$file" == *.spec.ts ]]; then
...
if [[ $# -gt 0 ]]; then
  ...
  while IFS= read -r -d '' file; do
    check_file "$file"
  done < <(find backend/src frontend/src -type f \( -name "*.py" -o -name "*.ts" -o -name "*.svelte" \) -print0 2>/dev/null || true)
fi
